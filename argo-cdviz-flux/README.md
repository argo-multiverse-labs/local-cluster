...
3. **FluxCD** - Secondary GitOps controller for CDEvents integration (sync-wave: 1)
4. **CDViz Database** - PostgreSQL 17.2 + TimescaleDB with CloudNativePG (sync-wave: 2)
5. **Flux Bootstrap** - FluxCD GitRepository and Kustomization CRs (sync-wave: 2)
6. **CDViz Collector** - Event collection service with kubewatch integration (sync-wave: 3)
7. **CDViz Grafana** - Dashboard provisioning to existing Grafana instance (sync-wave: 4)
